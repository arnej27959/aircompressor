...
/*
    ...
    {
        try {
            FileInputStream f = new FileInputStream("test.zst");
            FileOutputStream o = new FileOutputStream("test.out");
            InputStream i = new ZstdInputStream(f);
            byte[] buf = new byte[100 * 1024];
            int rl = 0;
            do {
                rl = i.read(buf);
                System.err.println("read " + rl + " bytes");
                if (rl > 0) {
                    o.write(buf, 0, rl);
                }
            } while (rl > 0);
